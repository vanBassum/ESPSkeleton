...
device never "opens" it, the client never replies, and it sits outside the
`SessionMux` (`OnChunk`/`OnSessionOpened`) machinery. The session-mux design already
...
client, a reboot), the `fd` lingers in `wsClients_` until a broadcast send fails and
...
- Evict the client from `wsClients_` *before* the send that would fail-and-log, or on
