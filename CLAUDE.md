...
Adding a **framework** manager: create the class, add it to `StruxProvider`, `StruxContext` (member *and* the ordered `Init()`), and `STRUX_SOURCES` + `INCLUDE_DIRS_LIST` in [main/CMakeLists.txt](main/CMakeLists.txt). Adding an **application** manager: the same, against `AppProvider`, `AppContext` and `APP_SOURCES` — and nothing in `strux/` is touched. [main/app/LedManager/](main/app/LedManager/) is the worked example: it owns the board LED — off at boot, toggled from the dashboard's LED card ([frontend/src/components/LedCard.tsx](frontend/src/components/LedCard.tsx)), optionally blinking — and along the way registers two settings, two commands and a telemetry point without a single edit to the framework. Delete it when the product has real features.
